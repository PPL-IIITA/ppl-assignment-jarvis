...
public class q5{
	...
	public static void main(String args[]) throws IOException{
		...
		for(i = 0 ;i < 20; i++)
		{
			for(j = i+1; j < 20; j++)
			{
				if(b[i].attractivness > b[j].attractivness) {
					boy = b[i];
					b[i] = b[j];
					b[j] = boy;
				}
		...
		for(c=0;c<i;c++){
			for(d=0;d<j;d++){
				if(b[c].is_eligible(g[d].expense,g[d].attar) && b[c].status.equals("Single") && g[d].status.equals("Single") && b[c].budget >= g[d].expense){
					b[c].girlf = g[d].name;
					g[d].boyf = b[c].name;
		...
		String coup = "\n ARE "+h+" THE BEST HAPPIEST COUPLE :"+"\n";
		try(BufferedWriter buffe=new BufferedWriter(new FileWriter(out,true))){
        	buffe.write(coup);
		
		for(x=0;x<h;x++){
			String df= new SimpleDateFormat("dd/MM/yy HH:mm:ss").format(new Time(System.currentTimeMillis()));
            String output = "\n"+ df+" "+gbcouple[x].boy.name+" and "+gbcouple[x].girl.name+"\n";
            System.out.println(output);
            try(BufferedWriter buffer=new BufferedWriter(new FileWriter(out,true))){
            	buffer.write(output);
