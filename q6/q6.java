...
/*! Sorting Gift according to price*/	
		...
		u = rand.nextInt(30)+1;
		breakup_couple bk = new breakup_couple(gbcouple, count, o,output,u);// Least n happy couples broke up
		String outp = "";
		outp = bk.break_print();
		for(i =0 ; i < o; i++)
		{
			for(j = 0;  j < 20; j++)
			{
				if(b[j].status == "Single" && gbcouple[i].girl.status == "Single"&& !((b[j].name).equals(gbcouple[i].boy.name)) && b[j].budget >= gbcouple[i].girl.expense && gbcouple[i].girl.attar >= b[j].min_attr_req) {
						...
						outp = outp.concat("\n"+fd+ " Day "+ 30/u +" "+"New Commitment : "+gbcouple[i].girl.name +"   and  "+b[j].name+" \n\n");//New commitments of broke up girls
